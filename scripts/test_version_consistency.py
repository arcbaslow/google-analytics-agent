...
.claude-plugin/plugin.json sat at 0.3.0 while pyproject.toml and the
changelog were at 0.5.0, because nothing checked them. This test is the
check. It complements check_release_version.py, which only runs at
release time and only compares the tag against pyproject.
"""
