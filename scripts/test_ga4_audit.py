...
def test_run_events_full_ecomm_taxonomy_no_findings_when_coverage_ok(monkeypatch):
    monkeypatch.setattr(ga4_audit.ga4_events, "list_events", lambda pid, days: {
        "rows": [
    ...
    monkeypatch.setattr(ga4_audit.ga4_events, "event_params_coverage", lambda pid, ev, days: {
        "event": ev, "coverage": {"currency": {"coverage_pct": 99}, "value": {"coverage_pct": 100}},
    })
...
def test_run_conversions_ok_when_two_key_events(monkeypatch):
    monkeypatch.setattr(ga4_audit.ga4_admin, "list_key_events", lambda pid: [
        {"eventName": "purchase"}, {"eventName": "generate_lead"},
    ])
    out = ga4_audit.run_conversions("123")
...
def test_run_property_flags_short_retention(monkeypatch):
    monkeypatch.setattr(ga4_audit.ga4_admin, "get_property_details", lambda pid: {
        "displayName": "Acme",
        "dataRetentionSettings": {"eventDataRetention": "TWO_MONTHS"},
    })
    monkeypatch.setattr(ga4_audit.ga4_admin, "list_data_streams", lambda pid: [
    ...
    monkeypatch.setattr(ga4_audit.ga4_admin, "list_custom_defs", lambda pid: {
...
def test_run_funnel_surfaces_leakiest_step(monkeypatch):
    monkeypatch.setattr(ga4_audit.ga4_funnel, "build_funnel", lambda **kw: {
        "property_id": kw["property_id"],
        "window_days": kw["days"],
        "steps": kw["steps"],
        "rates": {"aggregate": {
            "overall_conversion_pct": 2.1,
            "leakiest_step": {"from": "view_item", "to": "add_to_cart",
                              "users_dropped": 142580, "share_of_total_loss_pct": 64},
        }},
        "warnings": [],
    })
...
def test_run_attribution_flags_high_direct(monkeypatch):
    monkeypatch.setattr(ga4_audit.ga4_admin, "get_attribution_settings", lambda pid: {
        "reporting_attribution_model": "DATA_DRIVEN",
    })
    monkeypatch.setattr(ga4_audit.ga4_data, "run_report", lambda **kw: {
        "rows": [
...
def test_orchestrate_end_to_end(monkeypatch):
    ...
    right shape."""
    monkeypatch.setattr(ga4_audit.ga4_context, "build_property_context", lambda pid, force=False: {
        "status": "ok",
        "context": {
            "property_id": pid,
    ...
    monkeypatch.setattr(ga4_audit, "run_quality", lambda pid, days: ga4_audit._ok(
        "ga4-quality", "ok", [], {"confidence_label": "high"}))
    monkeypatch.setattr(ga4_audit, "run_events", lambda pid, days: ga4_audit._ok(
        "ga4-events", "ok", [], {"ecomm_events_present": ["view_item", "add_to_cart", "purchase"]}))
    monkeypatch.setattr(ga4_audit, "run_funnel", lambda pid, steps, days, check_postpayment: ga4_audit._ok(
        "ga4-funnel", "ok", [], {"steps": steps}))
    monkeypatch.setattr(ga4_audit, "run_conversions", lambda pid: ga4_audit._ok(
        "ga4-conversions", "ok", [], {"key_event_count": 2}))
    monkeypatch.setattr(ga4_audit, "run_attribution", lambda pid, days, primary_event: ga4_audit._ok(
        "ga4-attribution", "ok", [], {"called_with": primary_event}))
    monkeypatch.setattr(ga4_audit, "run_property", lambda pid: ga4_audit._ok(
        "ga4-property", "ok", [], {}))
    ...
    assert agent_names == [
        "ga4-context", "ga4-quality", "ga4-events", "ga4-funnel",
        "ga4-segments", "ga4-conversions", "ga4-attribution", "ga4-property",
    ]
...
def test_orchestrate_skips_attribution_without_key_events(monkeypatch):
    monkeypatch.setattr(ga4_audit.ga4_context, "build_property_context", lambda pid, force=False: {
        "status": "ok", "context": {"site": {"inferred": {"vertical": "other"}, "summary": "x"}},
    })
    monkeypatch.setattr(ga4_audit, "run_quality", lambda pid, days: ga4_audit._ok("ga4-quality", "ok"))
    monkeypatch.setattr(ga4_audit, "run_events", lambda pid, days: ga4_audit._ok("ga4-events", "ok", [], {"ecomm_events_present": []}))
    monkeypatch.setattr(ga4_audit, "run_conversions", lambda pid: ga4_audit._ok(
        "ga4-conversions", "no key events", [], {"key_event_count": 0}))
    ...
    agents_out, _ctx, vertical, _conf = ga4_audit.orchestrate(
        property_id="111", days=28, funnel_steps_arg=None,
        vertical_override=None, check_postpayment=False, refresh_context=False,
    )
...
def test_resolve_funnel_steps_uses_present_ecomm_subset():
    ...
    events_out = {"data": {"ecomm_events_present": [
