...
AGENT_OUTPUTS = [
    {
        ...
        "findings": [
            {"severity": "High", "title": "Direct share above industry p75",
             "detail": "42% of sessions land as (direct)/(none)",
             "metric": "direct_share", "metric_value": 0.42},
            {"severity": "Medium", "title": "Sampling visible on long-window queries",
    ...
    },
    {
        ...
        "findings": [
            {"severity": "Low", "title": "Overall conversion rate in line with vertical median",
             "metric": "conversion_rate", "metric_value": 0.018},
        ],
...
CONTEXT = {
    "property_id": "123",
    "primary_stream": {"stream_id": "9", "stream_name": "Web", "default_uri": "https://example.com"},
    "site": {
        "homepage": {"status": 200, "title": "Acme", "lang": "en", "server": "nginx"},
        "inferred": {"vertical": "ecommerce", "framework": "nextjs", "platform": "shopify", "is_spa": True},
        "sitemap": {"url_count_total_estimate": 1240, "page_types": {"product": 800, "blog_post": 220}},
...
def test_render_markdown_includes_property_context_section():
    md = ga4_report.render_markdown("123", AGENT_OUTPUTS, confidence="medium", context=CONTEXT, vertical="ecommerce")
...
def test_render_markdown_includes_confidence_and_vertical_in_header():
    md = ga4_report.render_markdown("123", AGENT_OUTPUTS, confidence="high", context=CONTEXT, vertical="ecommerce")
